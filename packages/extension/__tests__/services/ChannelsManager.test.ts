...
axiosMock
  .onGet('https://api.twitch.tv/helix/streams', { params: { user_id: [174955366] } }).replyOnce(200, {"data":[{"id":"27218318352","user_id":"174955366","game_id":"493244","community_ids":[],"type":"live","title":"SOLARY TEAM D'AMIS AVANT CE JEU","viewer_count":4263,"started_at":"2018-01-09T07:27:59Z","language":"en-gb","thumbnail_url":"https://static-cdn.jtvnw.net/previews-ttv/live_user_solary-{width}x{height}.jpg"}],"pagination":{"cursor":"eyJiIjpudWxsLCJhIjp7Ik9mZnNldCI6MX19"}})
  .onGet('https://api.twitch.tv/helix/streams', { params: { user_id: [174955366] } }).replyOnce(200, {"data":[],"pagination":{}})
  .onGet('https://api.twitch.tv/helix/games', { params: { id: "493244" } }).reply(200, {"data":[{"id":"493244","name":"Deceit","box_art_url":"https://static-cdn.jtvnw.net/ttv-boxart/Deceit-{width}x{height}.jpg"}]})
...
describe('Service - ChannelsManager', () => {
  ...
  describe('requestTwitchApi()', () => {
    it('should go online', async () => {
      ...
      expect(chrome.browserAction.setTitle).toHaveBeenCalledWith({ title: `SOLARY TEAM D'AMIS AVANT CE JEU\n\nJoue à Deceit devant 4263 viewers` });
    });
